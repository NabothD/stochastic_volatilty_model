...
if __name__ == "__main__":
    ...
    Nsteps =  len(prices) # trading days in a year
    maturity = T
    true_values = {'mu': 0.1, 'kappa': 1, 'theta': 0.05, 'sigma': 0.01, 'rho': -0.5, 'lambda': 1, 'mu_j': -0.3, 'sig_j': 0.2}
    ...
    vols = results['volatility_estimates_all']*100
    ...
    parameter_df.to_csv("parameter_samples_bates_real.csv", index=False)
    df2.to_csv("vol_samples_bates_real.csv", index=False)
    ...
    plt.xlabel('Time / years', fontsize=20)                   # <-- Set x-label font size
