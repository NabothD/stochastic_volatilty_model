...
def simulate_heston(
    ...
    v0,         # initial variance (v0 = sigma^2 if you prefer vol^2)
    mu,         # drift of S
    kappa,      # mean-reversion speed
    theta,      # long-term variance
    sigma_v,    # volatility of variance
    rho,        # correlation between dW1 & dW2
    T,          # total time horizon
    Nsteps      # number of discrete time steps
):
    ...
    Z1 = np.random.randn(Nsteps)
    Z2 = np.random.randn(Nsteps)

    for i in range(Nsteps):
        dW1 = np.sqrt(dt) * Z1[i]
        dW2 = rho * np.sqrt(dt) * Z1[i] + np.sqrt(1-rho**2)*np.sqrt(dt)*Z2[i]

        # Euler update for v:
        # dv_t = κ(θ - v_t) dt + σ sqrt(v_t) dW2
        v_t = v[i]
        v_next = v_t + kappa*(theta - v_t)*dt + sigma_v*np.sqrt(max(v_t,0.0))*dW2
        # enforce v_next >= 0
        ...
        S_t = S[i]
        S_next = S_t + mu*S_t*dt + np.sqrt(max(v_t,0.0))*S_t*dW1
        # Alternatively, some prefer the log-Euler approach for the price. 
