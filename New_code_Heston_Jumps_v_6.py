...
def refined_resample(V_tilde, weights):
    """
    Perform refined resampling of particles using inverse transform sampling,
    as described on page 11 (Equations (67)–(73)).

    Parameters:
      V_tilde : numpy array of raw particle values at time k (e.g., candidate volatility values)
      weights : numpy array of normalized weights corresponding to each particle (summing to 1)

    Returns:
      V_refined : numpy array of refined particles sampled from the continuous CDF
    """
    # 1. Sort particles and weights in ascending order.
    ...
    # 2. Compute the cumulative distribution function (CDF) from the sorted weights.
    # This is our continuous approximation of the empirical CDF.
    cdf = np.cumsum(W_sorted)
    
    # 3. Inverse transform sampling.
    # For each of N samples, generate a uniform random number and then use linear interpolation.
    N = len(V_tilde)
    u = np.random.uniform(0, 1, N)  # N uniform random numbers in [0, 1)
    V_refined = np.zeros(N)
    
    for i in range(N):
        # Find the first index such that cdf[index] >= u[i]
        idx = np.searchsorted(cdf, u[i])
        if idx == 0:
            # If the uniform sample falls in the first interval, assign the first sorted particle.
            V_refined[i] = V_sorted[0]
        else:
            # Linear interpolation between the particle at idx-1 and idx.
            cdf_lower = cdf[idx - 1]
            cdf_upper = cdf[idx]
            # Compute the fraction within the interval:
...
def heston_with_merton_jumps_estimation(
    ...
    for i in range(n_samples):
        ...
        for k in range(n):
            ...
            #   λ(kΔt) = sum_j [ J_j W_j ]
            lam_t_series[k] = np.sum(J * weights)  *1.0/n_particles  # if we used uniform W_j after resampling
        ...
        lam_j = np.sum(lam_t_series) * 1.0/n_particles
...
if __name__ == "__main__":
    ...
    b_sigma_0 = 0.026
    mu_psi_0 = -0.45
    tau_psi_0 = 1.0/math.sqrt(0.26**2)
    a_omega_0 = 1.03
    b_omega_0 = 0.05
